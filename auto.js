...
app.act.gcc.set = function(key) {
   ...
   * @param {string} args
   */
  function addOutput(complete, cancel, args) {
    args += ' --output_wrapper ' + fm.get('gcc.template');
    complete(args);
  }
 ...
 * @param {function(string, number=)} cancel
 */
app.act.gcc.readTemplate = function(complete, cancel) {
  act.fs.file.read(function(data) {
    fm.set('gcc.template', data);
    complete();
  }, cancel, 'lib/template.node');
};
